/**
 * Pulse Component - Updated with Creator Economy Integration
 * Combines tokenized content discovery with creator coin launching
 */

import { useState } from 'react'
import { Music, Sparkles } from 'lucide-react'
import { CreatorEconomyModule } from './CreatorEconomyModule'

type ClipCategory = 'gaming' | 'sports' | 'defi' | 'stream'
type PulseTab = 'discover' | 'creator-economy'
...
function ContentDiscoveryTab({ clips }: { clips: Clip[] }) {
  return (
...
export function Pulse({ clips = defaultClips }: PulseProps) {
  const [activeTab, setActiveTab] = useState<PulseTab>('discover')

  return (
    <div className="relative">
      {/* Tab Navigation */}
      <div className="mb-6 flex gap-2 border-b border-purple-300/20">
        {[
          { id: 'discover' as PulseTab, label: 'Discover', icon: Sparkles },
          { id: 'creator-economy' as PulseTab, label: 'Creator Economy', icon: Music },
        ].map((tab) => {
          const Icon = tab.icon
          return (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-3 border-b-2 font-bold uppercase text-xs tracking-wider transition ${
                activeTab === tab.id
      ...
      {/* Tab Content */}
      {activeTab === 'discover' && <ContentDiscoveryTab clips={clips} />}
